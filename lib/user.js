...
command.handler = function(argv) {
    ...
    Promise.props({
        ...
        .then(function(data) {
            ...
            //select broadcast by provided broadcast id
            if (argv.broadcast) {
                for (var i = 0; i < data.broadcasts.length; i++) {
                    var broadcast = data.broadcasts[i];
                    if (broadcast.broadcastId === argv.broadcast) {
                        data.res = {};
                        data.res.broadcast = broadcast;
                        break;
                    }
                }

                if (!data.res) {
                    throw new Error('unknown broadcast id `' + argv.broadcast + '` for user `' + data.username + '`');
                }
            }
            //manual broadcast selection
                ...
                for (var i = 0; i < data.broadcasts.length; i++) {
                    var broadcast = data.broadcasts[i];
                    choices.push({
                        name: '[' + (i + 1) + '] ' + broadcast.broadcastTitle + ' (' + broadcast.timeAgo + ' ago)',
                        value: broadcast
                    });
                }
